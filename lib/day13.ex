defmodule Day13 do
  ...
  def find_greatest_line2(lines) do
    ...
    case {h, v} do
      {{h_idx1, _h_idx2}, {v_idx1, _v_idx2}} -> {:h, h_idx1}
      {:not_found, {v_idx1, _v_idx2}} -> {:v, v_idx1}
      {{h_idx1, _h_idx2}, :not_found} -> {:h, h_idx1}
      _ -> {:v, 0}
    end
  ...
  defp find_line_22([_ | []], _) do
    :not_found
  end
  defp find_line_22([{elems1, idx1}, {elems2, idx2} | tail], lines) do
    cond do
      compare_lines(elems1, elems2, 0) in [0, 1] and index_can_propagate2(lines, {idx1, idx2}) -> {idx1, idx2}
      true -> find_line_22([{elems2, idx2} | tail], lines)
    end
  end

  def compare_lines([], _, acc) do
    acc
  end
  ...
  def index_can_propagate2(lines, {idx1, idx2}) do
    index_can_propagate2(lines, {idx1-1, idx2-1}, 1, true)
  end
  @spec index_can_propagate2(list(), {number(),number()}, number(), boolean()) :: boolean()
  def index_can_propagate2(lines, {idx1, idx2}, idx, flag) do
    cond do
      idx1 - idx < 0 or idx2 + idx === length(lines) -> true
      compare_lines(Enum.at(lines, idx1 - idx), Enum.at(lines, idx2 + idx), 0) === 0 -> index_can_propagate2(lines, {idx1, idx2}, idx + 1, flag)
      compare_lines(Enum.at(lines, idx1 - idx), Enum.at(lines, idx2 + idx), 0) === 1 and flag -> index_can_propagate2(lines, {idx1, idx2}, idx + 1, false)
      true -> false
    end
  end
end
